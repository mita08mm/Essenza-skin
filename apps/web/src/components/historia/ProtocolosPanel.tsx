  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const itemsParaBackend = items.map(item => ({
        ...
        aplicacion: item.instrucciones,
        frecuencia: item.instrucciones,
      }));

      const payload = { pacienteId, nombre: nombre.trim(), items: itemsParaBackend };
      ...
      if (!response.ok) {
        // ✅ FIX: se separó el parse del throw para que el catch interno
        //    no atrape el error que acabamos de lanzar
        ...
        try {
          ...
          if (errorData.details && Array.isArray(errorData.details)) {
            mensajeError = errorData.details
          ...
          mensajeError = responseText || mensajeError;
        }
    ...
    <>
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          {/* ✅ type="button" explícito para evitar submit accidental */}
          <button
            type="button"
        ...
        <div className="space-y-4">
          {isLoadingProtocolos ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-xs text-gray-300">Haga clic en + para agregar</p>
            </div>
          ) : (
          ...
          {protocolos.length > 5 && (
            <button type="button" className="w-full text-xs text-gray-500 hover:text-gray-700 pt-3 transition-colors">
      ...
      {/* ✅ Modal: clic en backdrop cierra, clic dentro NO cierra */}
      {showModal && (
        <div
